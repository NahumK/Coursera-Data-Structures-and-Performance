...
public abstract class Document 
{
	...
	protected int countSyllables(String word)
	{
		...
		{
			char letter = wordTmp.charAt(index);
			...
			if(vowels.indexOf(letter) >= 0)
			{
				...
				{
					char precLetter = wordTmp.charAt(index - 1);
					if(vowels.indexOf(precLetter) < 0)
						count++;
				}
			}
		}
		
		char beforLastLetter = ' ';
		if(size - 2  >= 0)
			beforLastLetter = wordTmp.charAt(size - 2);

		if((count > 1) && (word.charAt(size - 1) == 'e') && (vowels.indexOf(beforLastLetter) < 0))
			count--;
	...
	public double getFleschScore()
	{
		...
		double wordsTotal = this.getNumWords();
		double sentencesTotal = this.getNumSentences();
		double syllablesTotal = this.getNumSyllables();
		
		score = 206.835 - 1.015 * (wordsTotal / sentencesTotal) - 84.6 * (syllablesTotal / wordsTotal);
