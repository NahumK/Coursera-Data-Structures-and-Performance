...
public class EfficientDocument extends Document 
{
	...
	private void processText()
	{
		...
		int countWords = 0, countSentences = 0, countSyllables = 0, index = 1;
		...
		for(String token : tokens)
		{
			...
			{
				countWords++;
				countSyllables += this.countSyllables(token);
				if(index == size)
					countSentences++;
			}
			index++;
		...
		this.numSentences = countSentences;
		this.numSyllables = countSyllables;
