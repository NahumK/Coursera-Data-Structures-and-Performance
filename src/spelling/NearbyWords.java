...
public class NearbyWords implements SpellingSuggest 
{
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) 
	{
		...
		for(int index = 0; index <= len; index++)
		{
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) 
			{
				...
				// letters in the String
				String temp = s.substring(0, index) + 'x' + s.substring(index);
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) 
	{
		...
		for(int indexToDel = 0; indexToDel < len; indexToDel++)
		{
			String toAdd= "";
			for(int index = 0; index < len; index++)
			{
				if(index != indexToDel)
					toAdd += s.charAt(index);
			}
			// if the item isn't in the list, isn't the original string, and
			// (if wordsOnly is true) is a real word, add to the list
			if(!currentList.contains(toAdd) && (!wordsOnly || dict.isWord(toAdd))) 
				currentList.add(toAdd.toString());
		}
	}
